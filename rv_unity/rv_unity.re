...
プレイヤーに"撃った手ごたえ"を感じてもらうために着弾点に爆発のようなエフェクトを付けます．
エフェクトはUnityのパーティクルシステムという機能で作成することも可能ですが，
今回はAsset Storeで無料の素材をお借りしましょう．
...
そしてhitEffectはpublicで宣言しているため，Main Cameraのインスペクタから手動でアタッチします．
先ほどインポートしたアセットの「JMO Assets>WarFX>_Effects>Explosions>WFX_Explosion Small」をドラッグ&ドロップでアタッチします．

実行してみるとわかるのですが，爆発が大きすぎて不自然です．そこでエフェクトのScaleをx，y，z全て0.3にしますと適切なサイズになります．
エフェクトを変えてもプログラム自体は変える必要ないので他のエフェクトアセットを使ったりエフェクトのプロパティを変えるなど，
好みに合うよう試してみるのもいいでしょう．
//image[1_12][スクリプトにアタッチしたエフェクト][scale=0.8]{
//}
...
===[column] 正常に動かない？
スクリプトを書いて「これでゲームが動くぞっ」と実行してもエラーが出たり，思った通りに動かないということはよくあります．
「プログラミング難しい」「Unity何も分からん」など思ってしまいますが，一発で動くことのほうが少ないです．
エラー文での検索や，「Debug.Log」関数で原因を探してみましょう．
...
駅の裏側はゲームをプレイしていて見えないため作りこまなくて良く，
逆に奥のビルはプレイヤーから丁度良く見えるように浮かせています．

ゲームをしていて見えないところにも，こだわってオブジェクトを置いたり
高画質なテクスチャを貼ると，パフォーマンスが落ちてしまい本末転倒です．
ゲームのマップは舞台装置ですから必要最低限でいいのです．
...
ゲーム開始ボタンやタイトルテキスト，イラストなどをCanvasに配置します．
//emlist[Zombie.cs][c#]{
...
//image[4_6][ゲーム開始ボタンに関数呼び出しを設定][scale=0.7]{
//}

//image[4_5][タイトルと，開始ボタンを押したときの遷移][scale=1]{
//}
...
ビルドされたフォルダ内のexeファイルでゲームを実行できます．

「Graphics」でウィンドウサイズやクオリティを設定し，「Play!」をクリックすると開始できます．
無事遊ぶことができたらゲーム完成です！

= Wiiリモコンで遊べるようにする
ゲームが完成したらいよいよ"アーケード"ゲームにするべくWiiリモコンの入力を対応させます．
とはいっても，実は特殊なことをするわけではなくWiiリモコンをWindowsマウスカーソルに変換するフリーソフト「TouchMote」を使うだけです．
アーケードゲームと言うと聞こえはいいですが，実は簡単ですね．

WiiリモコンはBluetoothでゲーム機と通信しており，WindowsPCでもペアリングが可能です．

ちなみに，センサーバーと呼ばれるディスプレイに設置するアレは赤外線のLEDが入っているだけで，
リモコンがその光で向ける方向を認識しています．
"センサー"と名前が付いてはいますが受信機能はありません．
仕組みはシンプルなためセンサーバーを自作する人もいるそうです．

=== TouchMoteのインストール
TouchMoteはWindows上のマウスカーソルやキーボード入力に，
Wiiリモコンのポインターやボタンを割り当てることのできるフリーソフトです．

公式サイト@<fn>{touchmote}からダウンロードしインストールします．

=== Wiiリモコンのペアリング
WiiリモコンをBluetooth接続でペアリングします．
Wiiリモコンは本来PCに繋げる想定はしていないため少々癖があり，OSやBluetoothドライバにもよって変わることがあります．
この項では筆者のthinkpad x250内蔵Bluetooth，Windows10環境での設定方法を載せます．

手順を追って説明します．

1.デスクトップのツールバーのBluetoothアイコンを右クリックし「パーソナルエリアネットワークへ参加」を選択．

//image[5_1][Wiiリモコンのペアリング手順(1)][scale=0.55]{
//}
...
//image[5_3][Wiiリモコンのペアリング手順(3)][scale=0.55]{
//}
...
//image[5_4][ペアリングしたらマウスとして使えるのでmspaintも遊べる][scale=0.35]{
//}
...
そこで，スマートフォンやデジカメのカメラを使います．
赤外線は肉眼では見えませんが，これらのカメラでは映りますので確認したいときに通して見てください．
中にはiPhone6など，赤外線カットフィルターで見えないものもあります．

= 余談

== ゲームをさらに面白くしよう
無事ゲームを完成させプレイした皆さんはお気づきでしょう．ゲーム性がイマイチだということに！
ひたすら現れてくるゾンビを撃つだけで単調になってしまっています．

そこで，ゾンビにHPを設定してヘッドショットを一撃にしたり，ゾンビ犬や鳥など特殊なキャラクタを登場させて
アクセントを加えるのもいいでしょう．さらに致命的な点としてBGMや効果音がありません．
射撃音やゾンビのうめき声は勿論，ヘッドショットで特別な効果音があると爽快感も増すでしょう．

想像が膨らんできましたか？
本書では基盤となるゲームは実装しましたが，その後どう発展させていくかはあなたの自由です．
「適切な難易度はどれくらいだろう」など遊ぶ人のことを考えて
試行錯誤するのもゲーム制作の醍醐味ですので，ぜひチャレンジしてほしいと多います(これは執筆しきれなかった逃げ)．

===[column] UIの壁
ゲーム制作者によくあることは，ストーリーやキャラクター，オリジナル要素などシステムに凝りすぎて
UIや操作性などプレイヤーの遊びやすさという点を軽視してしまうことです．
しかし，どんなにいいゲームもこのUIの壁を超えないと評価してもらえないことがあります．
我々の想像する以上に視覚や直感からの印象は大きく，裏を返せばシンプルなゲームでも遊びやすさやUIの工夫を施せば
高評価も狙えるということです．プログラミングだけでなく，「UI/UX」の視点でもゲーム制作をしてみましょう．
(これは罠で，筆者も学んでいかないといけない)
...
フットペダルは「REVIVE USB@<fn>{reviveUSB}」という，
ピンに電気スイッチを繋いで任意のキー入力に変換できるデバイスを用いました．

翌年はハンドル型wiiリモコンでレースゲームを作り，フットペダルはアクセル，ブレーキとして使いました．
Wiiは今では古いゲーム機かもしれませんが，加速度センサ，ジャイロセンサ，ポインタとスイッチなど
コントローラーとして非常に優れた機能を持ち，ゲーム制作に無限の可能性を与えてくれます．

文化祭は僅かな時間でゲームシステムを理解し，誰でも遊びやすくする工夫が必要です．
そのような点で，コントローラーで簡単に遊べる「Wiiパーティ」や「1-2-Switch」などのパーティゲームは
多くのヒントを得られそうです．

//image[6_1][REVIVE USBと自作フットペダル][scale=0.95]{
//}
//image[6_2][シューティングゲーム「高専危機」][scale=0.95]{
//}

//footnote[reviveUSB][http://bit-trade-one.co.jp/product/assemblydisk/revive-usb/]

= あとがき
およそ1週間の開発・執筆期間を経て無事完成させることができました．
ゲーム制作は趣味でずっと行っていたのですが，本書のような技術的な話のアウトプットは初めてでした．
本来はブログなどに忘備録としてマメに書き，同人誌ではそれらをまとめるという手法を取る方が多いのですが，
私の場合ミニゲームを作りながら並行して原稿を執筆していたため，
効率も悪く制作記みたいになってしまったかもしれません．

不特定多数の方に読んで頂ける技術同人誌の執筆では，どこまで詳細に書くべきかという点で
試行錯誤しました．初心者にとって分かりにくい本だったら申し訳ないです．

単にプログラムの知識だけでなく，技術本としての構成や印刷所への入稿，
データのフォーマット等初めて知ることが多くあり，失敗をしながらも勉強になりました．

本書の素敵な表紙イラストは私と同じくNitKitコンピュータ研究部に所属するみかんのつぶ(@mikannotsubu2)さんに
描いていただきました．私が「塗り足し？トンボ？dpi？何も分からない」となり
絵師の方に対して失礼な知識レベルの中，完璧に仕上げてくれて本当に感謝です．
さらに驚くべきことにペンタブが壊れていたようでスマホでイラストを描いてくれていたということです．
どういうこと？？？？る

執筆中にはTwitterにてコードレビューや文章修正，感想などを呼びかけたら想像以上の方に協力をくださり，
驚くとともに本当に嬉しかったです．
最早私一人ではなく，多くの方によって作り上げた本といっても過言ではありません．

元々技術同人誌を書こうと思ったのは，他高専の方が技術本を作っているのを見て興味を持ったのがきっかけでした．
後は何か自分の活動を分かりやすい形として残したいという理由です．
まあ技術同人誌自体結構自己満足なところがあると思います．
皆さんもぜひ軽率に同人誌を作ってほしいです．布教ですがRe:Viewという執筆システムがおすすめです．
なんか適当に書いてたら勝手に本の形になって出力されます(マジ)．

ただタイトルの改行の仕方が分からず，空白文字で調整してしまいました．
奥付のタイトル部分に不自然な空きがあるのはそれのせいです．デザイナーの方が発狂しそうな事案ですが今日から
僕もスペース中央揃えマンですよろしくお願いします．

この本を読んでゲーム制作に興味を持ってくれたり，楽しさに触れてくれたら本当に嬉しいです．
あとがきもこの辺にして終わろうと思います．しっかりと1ページに収めないといけませんからね．
